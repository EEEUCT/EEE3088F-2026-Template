...
def write_c_files(test_cases, folder_path):
    ...
    with open(h_path, 'w') as h, open(c_path, 'w') as c:
        ...
        h.write(" * These vectors are generated by a Python script (perfect math) to serve as\n")
        h.write(" * the Ground Truth for your C implementation.\n")
        h.write(" * \n")
        h.write(" * Usage:\n")
        h.write(" * 1. Use the macros below to configure your algorithm (N, FS, Spacing).\n")
        h.write(" * 2. In your test harness, pass these arrays to calculate_doa_angle_2mic().\n")
        h.write(" * 3. Verify that your output matches the angle in the variable name.\n")
        h.write(" */\n\n")
        ...
        for angle, name, snr, dc, amp in test_cases:
            ...
            # For autograding: If this is a secret test, export the expected angle as a macro
            if "test_secret" in name:
                suffix = name.replace("test_secret", "").upper()
                h.write(f"/* Autograder Secret Hook{suffix} */\n")
                h.write(f"\n#define AUTOGRADER_SECRET_ANGLE{suffix} {int(round(angle))}\n")
...
if __name__ == "__main__":
    ...
    cases = [
        ...
        (0, "test_0_deg_noisy", 10, 0, 2000),     # Robustness
